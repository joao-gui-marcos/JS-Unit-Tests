...
function compareConsumption(i, menuArray, j, orders) {
  let sum = 0;
  if (orders[i] === menuArray[j][0]) {
    sum += menuArray[j][1];
  }
  return sum;
}
...
const restaurant = {
  ...
  pay() {
    const fullMenu = { ...restaurant.fetchMenu.food, ...restaurant.fetchMenu.drink };
    const menuArray = Object.entries(fullMenu);
    let sum = 0;
    for (let i = 0; i < restaurant.consumption.length; i += 1) {
      for (let j = 0; j < menuArray.length; j += 1) {
        sum += compareConsumption(i, menuArray, j, restaurant.consumption);
      }
    }
    return sum;
  },
